...
def load_data(datatype, seq):
	...
	labels = np.loadtxt(f'./{datatype}/all_{seq}_{datatype}_lbl')
	encoded_seq = np.loadtxt(f'./{datatype}/all_{seq}_{datatype}')
...
def training_process(x_train,y_train,x_test,y_test, datatype=''):
	...
	model_data = model.save(f'./CNN_{accuracy}_{datatype}.h5')
	print(model.summary())
	keras.utils.plot_model(model, f"./model_summary_{datatype}.png", show_shapes=True)
	...
	plt.savefig(f"./accuracy_{datatype}.png")
	...
	with open(f'./file_metrics_{datatype}.txt', 'w') as fl:
