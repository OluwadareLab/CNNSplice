...
Length = 400  # length of sequence window
dimensions = 1
...
def training_process(x_train,y_train,x_test,y_test, datatype = ''):
	...
	for train, test in kfold.split(X, Y):
		...
		model.save_weights(f"./weight_{datatype}.h5")
	...
	with open(f'./file_metrics_{datatype}', 'w') as fl:
...
def app_init():
	...
	parser.add_argument("-e", "--encoded_seq", str=str, metavar='FILE', required=True, help="one-hot encoded genome sequence data file")
